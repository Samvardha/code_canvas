...
import { getCollabFeed, Post } from "@/lib/api/posts";
import { 
  PlusSquare, 
  Loader2,
  Users
...
export default function CollabFeedPage() {
  ...
  const fetchPosts = React.useCallback(async (userToken: string) => {
    try {
      setLoading(true);
      const data = await getCollabFeed(userToken);
      setPosts(data.posts);
    } catch (err) {
      console.error("Failed to fetch collab signals:", err);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black">
        <div className="font-mono text-accent text-sm uppercase tracking-widest font-bold animate-pulse flex items-center gap-3">
          <div className="w-3 h-3 bg-accent rotate-45" />[ SYNCING_COLLABS ]
        </div>
      </div>
    );
  }
          ...
          <header className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-border p-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
               ...
               >
                 ...
                 <span className="text-[9px] font-mono font-bold uppercase tracking-widest">New_Collab</span>
               </button>
            </div>
          </header>
